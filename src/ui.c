...
void synth_ui_add_item(synth_ui_t *synth_ui, char *name, volatile int32_t *value, bool active){
    ...
    synth_ui_item_t *new_item = &synth_ui->menu_items[synth_ui->item_count];
        ...
        lv_obj_set_align(new_item->label, LV_ALIGN_TOP_LEFT);
        lv_obj_set_x(new_item->label, PADDING_LEFT);
        lv_obj_set_y(new_item->label, (synth_ui->item_count * ROW_SIZE) + PADDING_TOP);
        synth_ui->item_count++;
}
